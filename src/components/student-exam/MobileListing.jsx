    ...
    return (
      <div className="md:hidden bg-white rounded-xl border border-gray-200">
        <div className="text-center py-16 px-4">
          ...
          <p className="text-sm text-gray-500">{t("studentExams.listEmptyHint")}</p>
        </div>
      </div>
    );
  }
          ...
          >
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-3">
                {(exam.class || exam.section) && (
                  <div className="flex items-center gap-1 text-xs text-gray-500">
                    ...
                    <span>{t("studentExams.classPrefix")} {exam.class}{exam.section && ` · ${exam.section}`}</span>
                  </div>
                )}
                <div className="flex items-center gap-1 text-xs text-gray-500">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <span>{t("studentExams.subjectCount", { count: subjectCount })}</span>
                </div>
              </div>
            </div>
            ...
            <div className={`px-4 py-2 ${config.bgAccent} border-t border-gray-100 flex items-center justify-between`}>
              <div className="flex items-center gap-1.5 text-xs text-gray-500">
                ...
                <span className="font-medium">{dateLabel || t("studentExams.dateTbd")}</span>
              </div>
