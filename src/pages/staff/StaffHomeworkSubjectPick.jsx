...
export default function StaffHomeworkSubjectPick() {
  ...
  useEffect(() => {
    if (subjects.length === 1 && sectionId) {
      const sid = encodeURIComponent(subjects[0].subject_id);
      navigate(`/staff/homework/section/${sectionId}/subject/${sid}`, { replace: true });
    }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-4 md:p-6">
      ...
      <h1 className="text-xl font-bold text-gray-900">{sectionTitle}</h1>
      <p className="mt-1 text-sm text-gray-600">Choose a subject to view or create homework.</p>
        ...
        {subjects.map((s) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => {
                const sid = encodeURIComponent(s.subject_id);
                navigate(`/staff/homework/section/${sectionId}/subject/${sid}`);
              }}
              className={`w-full flex items-center gap-4 bg-white rounded-xl border border-gray-200 border-l-4 ${border} px-4 py-3.5 cursor-pointer hover:shadow-sm hover:-translate-y-0.5 active:scale-[0.99] transition-all duration-150`}
            >
