...
function topicStatusConfig(status) {
  switch (status) {
    case "COMPLETED":
      return { Icon: CheckCircle2, color: "text-green-500", badge: "bg-green-100 text-green-700", gradient: "from-green-500 to-emerald-600", label: "Completed" };
    case "IN_PROGRESS":
      return { Icon: Zap, color: "text-blue-500", badge: "bg-blue-100 text-blue-700", gradient: "from-blue-500 to-indigo-600", label: "In Progress" };
    case "SKIPPED":
      return { Icon: MinusCircle, color: "text-gray-400", badge: "bg-gray-100 text-gray-500", gradient: "from-gray-400 to-gray-500", label: "Skipped" };
    default:
      return { Icon: Clock, color: "text-amber-400", badge: "bg-amber-100 text-amber-700", gradient: "from-primary-600 to-primary-800", label: "Upcoming" };
  }
}
...
function FileUploadRow({ fileUrl, fileName, uploading, onFile, onRemove }) {
  return fileUrl ? (
    <div className="flex items-center gap-2 rounded-lg border border-border bg-gray-50 px-3 py-2 text-sm">
      ...
      <button type="button" onClick={onRemove} className="text-xs text-gray-400 hover:text-red-500 flex-shrink-0">Remove</button>
    </div>
  ) : (
...
function AssignmentForm({ topicId, assignment, onSaved, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-amber-200 bg-amber-50/40 p-4 space-y-3">
      <p className="text-sm font-semibold text-gray-900">{isEdit ? "Edit Assignment" : "New Assignment"}</p>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" loading={submitting || uploading}>{isEdit ? "Save changes" : "Add assignment"}</Button>
      </div>
    </form>
  );
}
...
function QuizForm({ topicId, quiz, onSaved, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-violet-200 bg-violet-50/40 p-4 space-y-3">
      <p className="text-sm font-semibold text-gray-900">{isEdit ? "Edit Quiz" : "New Quiz"}</p>
      ...
      <div className="flex justify-end gap-2">
        <Button type="button" variant="secondary" onClick={onCancel}>Cancel</Button>
        <Button type="submit" loading={submitting || uploading}>{isEdit ? "Save changes" : "Add quiz"}</Button>
      </div>
    </form>
  );
}
...
function MaterialForm({ topicId, onSaved, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-teal-200 bg-teal-50/40 p-4 space-y-3">
      <p className="text-sm font-semibold text-gray-900">Upload Study Material</p>
      {fileUrl ? (
        ...
        <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-dashed border-teal-200 bg-teal-50/50 px-4 py-6 text-sm text-gray-500 hover:bg-teal-50 transition-colors">
          <div className="text-center">
            ...
            <p className="font-medium text-gray-700">{uploading ? "Uploading…" : "Click to upload file"}</p>
            <p className="text-xs text-gray-400 mt-0.5">PDF, DOC, PPT, images, etc.</p>
          </div>
      ...
      <div className="flex justify-end gap-2">
        <Button type="button" variant="secondary" onClick={onCancel}>Cancel</Button>
        <Button type="submit" loading={submitting || uploading} disabled={!fileUrl}>Upload</Button>
      </div>
    </form>
  );
}
  ...
  if (error || !topic) {
    return (
      <div className="flex min-h-0 flex-1 flex-col p-4">
        ...
        <p className="text-sm text-red-600">{error || "Topic not found."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
        ...
        <div className="flex-1 min-w-0">
          ...
          {topic.chapter_title && (
            <p className="text-xs text-gray-500 truncate">
              {topic.chapter_number != null ? `Chapter ${topic.chapter_number}: ` : ""}{topic.chapter_title}
            </p>
          )}
        </div>
        <button
        ...
        <div className="max-w-2xl mx-auto p-4 pb-24 space-y-4">
          ...
          <div className={`bg-gradient-to-br ${sc.gradient} rounded-2xl p-5 text-white`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-4">
              <div className="bg-white/15 rounded-xl p-2.5">
                <p className="text-xs opacity-70 mb-0.5">Status</p>
          ...
          >
            ...
            {assignments.length === 0 && assignmentForm === null && (
              <p className="text-center text-sm text-gray-400 py-4">No assignments yet.</p>
            )}
            {assignments.map((a) => (
              <div key={a.id}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {a.file_url && (
                        <a href={a.file_url} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-primary-600 hover:underline">
                          <Paperclip className="h-3 w-3" />Attachment <ExternalLink className="h-3 w-3" />
                        </a>
                      )}
                    </div>
          ...
          >
            ...
            {quizzes.length === 0 && quizForm === null && (
              <p className="text-center text-sm text-gray-400 py-4">No quizzes yet.</p>
            )}
            {quizzes.map((q) => (
              <div key={q.id}>
                {quizForm?.id === q.id ? (
          ...
          >
            ...
            {materials.length === 0 && materialForm === null && (
              <p className="text-center text-sm text-gray-400 py-4">No study materials yet.</p>
            )}
            {materials.map((m) => (
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} className="max-w-sm">
        <h3 className="text-base font-semibold text-gray-900">Remove this item?</h3>
        <p className="mt-1.5 text-sm text-gray-500">This action cannot be undone.</p>
